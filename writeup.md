...
A. The first architecture chosen was the same LeNet-5 as in the course, because I thought it works also for images. In addition, both of them process the similar size of input images.
...
A. With the initial architecture, the network was overfitting. Therefore, I added a dropout to each fully connected layer. However, the overfitting was not improved. Both training and validation accuracy were not improved either.
...
A. In the end, increasing the depth of the convolution layers helped me improve the validation accuracy. Before changing the depth, I mainly tuned learning rate, number of EPOCS, number of batches but they did not give the required accuracy.
...
I am not sure if the following information is require in my case, but since my architecure is based on LeNet-5, I still wrote them as below.
* What architecture was chosen?  
A. LeNet-5 although the depth of the convolution layers
...
A. The accuracy of the training, the validation and the test set all reached over 93%.
...
The fourth image of a slippery road sign might be difficult to classify because the actual image of this is reshaped 32x32 by shrinking width instead of cropping it.
...
For the first image, the model is sure that this is a right-of-way sign (probability of 1.00), and the image does contain a right-of-way sign. The top five soft max probabilities were  
...
| 5.59e-13	      		| Road narrws on the right					 	|
| 3.36e-16				| General caution      							|

For the second image, the model is sure that this is a 20 km/h sign (probability of 0.99), and the image does contain a 20 km/h sign. The top five soft max probabilities were  
...
For the third image, the model is sure that this is a roundabout sign (probability of 0.99), and the image does contain a roundabout sign. The top five soft max probabilities were  
...
For the fourth image, the model is sure that this is a right-of-way sign (probability of 0.94), however the image contains slippery road sign. This is probably because the model cannot detect the image inside of the diagonal. Compared to other images, the probablity of the 2nd and 3rd predictions are relatively high. In addition, this may be because the number of training samples of slippery roads are relatively small. The top five soft max probabilities were
...
For the fifth image, the model is sure that this is a priority road sign (probability of 1.00), and the image does contain a priority road sign. The top five soft max probabilities were
